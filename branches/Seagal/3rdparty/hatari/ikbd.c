// Hatari's ikbd.c (1.6.1) adapted for Steem SSE
...
static void IKBD_AddKeyToKeyboardBuffer_Real(Uint8 Data, int nAciaCycles)
{
	...
	if(IKBD_ExeMode)
	{
		IKBD_ExeMode=0; // hack in hack...
		keyboard_buffer_write(Data); // Steem handling
		IKBD_ExeMode=TRUE;
	}
#endif
...
static void IKBD_CustomCodeHandler_CommonBoot ( Uint8 aciabyte )
{
	...
	if ( i < sizeof ( CustomCodeDefinitions ) / sizeof ( CustomCodeDefinitions[0] ) )	/* found */
	{
...
#if defined(STEVEN_SEAGAL) && defined(SS_VAR_PROG_ID)
		switch(i)
		{
		case 0:
			SetProgram(FROGGIES);
		...
		case 1:
			...
			break;
		case 2:
			SetProgram(DRAGONNELS);
			break;
		default:
			TRACE("IKBD Custom program??\n");
		}
#endif
...
#if defined(STEVEN_SEAGAL) && defined(SS_IKBD_FROGGIES)
	// arrows repeat
	if(SpecificHacks &&
		(ScanCodeState[ 0x4b ] || ScanCodeState[ 0x4d ]
		|| ScanCodeState[ 0x48 ] || ScanCodeState[ 0x50 ]))
	{
		IKBD_AddKeyToKeyboardBuffer_Real(0, ACIA_CYCLES);
		IKBD_AddKeyToKeyboardBuffer_Real(0, ACIA_CYCLES);
	}
#endif
...
static void IKBD_CustomCodeHandler_DragonnelsMenu_Read ( void )
{
	...
	// Hack: add keyboard selection!
	if(SpecificHacks)
	{
		if ( ScanCodeState[ 0x48 ] )	res |= 0xfc;		
		if ( ScanCodeState[ 0x50 ] )	res |= 0x04;		
		if ( ScanCodeState[ 0x39 ] )	res = 0x80;
        }
#endif
	IKBD_AddKeyToKeyboardBuffer_Real(res, ACIA_CYCLES);
