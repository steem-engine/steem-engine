// Hatari's video.c (1.6.1) adapted for Steem SSE
...
void	Video_ConvertPosition ( int FrameCycles , int *pHBL , int *pLineCycles )
{
	...
	// We use Steem's cpu_timer_at_start_of_hbl instead
	*pLineCycles=ABSOLUTE_CPU_TIME-cpu_timer_at_start_of_hbl;
	if((*pLineCycles)&2)
		(*pLineCycles)+=2; // rounding now
	*pHBL = nHBL;
	ASSERT(*pLineCycles>=0);
	if ( *pLineCycles > nCyclesPerLine )		/* reading on the next line, but HBL int was delayed */
	{
		*pHBL = nHBL+1;
		*pLineCycles -= nCyclesPerLine;
	}

#else // Hatari original
